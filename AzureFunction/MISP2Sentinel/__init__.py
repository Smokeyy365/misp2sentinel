...
from MISP2Sentinel.RequestObject import RequestObject, RequestObject_Event, RequestObject_Indicator, RequestObject_ThreatActor, RequestObject_Identity, RequestObject_Relationship
...
def push_to_sentinel(tenant, id, secret, workspace):
    ...
    config.ms_auth[TENANT_ID] = tenant
    config.ms_auth[CLIENT_ID] = id
    config.ms_auth[CLIENT_SECRET] = secret
    config.sentinel_workspace_id = workspace
    logger.info("Tenant: %s", tenant)
    logger.info("Client ID: %s", id)
    logger.info("Workspace ID: %s", workspace)
    obfuscated_secret = secret[:-5] + '*' + '*' + '*' + '*' + '*'
    logger.info("Client Secret (obfuscated): %s", obfuscated_secret)
...
def pmain(logger):
    ...
    # Single-tenant mode
    tenant = config.ms_auth[TENANT_ID]
    id = config.ms_auth[CLIENT_ID]
    secret = config.ms_auth[CLIENT_SECRET]
    workspace = config.sentinel_workspace_id
